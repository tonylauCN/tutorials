...
local function _do_cmd(self, ...)
    ...
    local req_str = "request: " .. table.concat(req)
...
local function _check_subscribed(self, res)
    if type(res) == "table" and (res[1] == "unsubscribe" or res[1] == "punsubscribe") and res[3] == 0 then
        self._subscribed = false
    end
end
...
for k, v in pairs(extra_commands) do
    if type(v) == "table" then
        if not _M[k] then
            _M[k] = {}
        end
        for i, cmd in ipairs(v) do
            local key = k .. "." .. cmd
            _M[k][cmd] = function(self, ...)
                local res, err = _do_cmd(self, key, ...)
                if not res then
                    print(err)
                end
                return res, err
...
for i, res in ipairs(results) do
    if type(res) == "table" then
        if res[1] == false then
            ...
            ngx.say("success to run command ", i, ": ", res[2])
        end
    else
        -- process the scalar value
        ngx.say("success to run command ", i, ": ", res[1])
    end
end
